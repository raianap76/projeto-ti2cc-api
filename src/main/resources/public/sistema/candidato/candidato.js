...
const deleteClient = (index) => {
  let url = `http://localhost:6789/candidato/${index}`;
...
const readClient = () => {
  let data = fazGet("http://localhost:6789/candidato");
  console.log(data);
...
const saveClient = () => {
  ...
  if (!openModalEditar) {
    if (isValidFields()) {
      let url = "http://localhost:6789/candidato";
    ...
    let url = `http://localhost:6789/candidato/${client.candidato_id}`;
    console.log("Body=", client);
    let request = new XMLHttpRequest();
